...
public class OpenShiftJsonDTOFactory implements IRestResponseFactory {
	...
	private CartridgeResourceDTO createCartridge(ModelNode cartridgeNode, Messages messages)
		...
		final List<String> tags = createTags(cartridgeNode.get("tags"));
		return new CartridgeResourceDTO(name, displayName, description, CartridgeType.safeValueOf(type), url, properties, links, messages, tags);
	}

	private List<String> createTags(ModelNode tagsNodes) {
		// TODO Auto-generated method stub
		List<String> tags = new ArrayList<String>();
		try {
			if(tagsNodes.isDefined()){
				for(ModelNode node : tagsNodes.asList()){
