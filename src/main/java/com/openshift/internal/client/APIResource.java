...
public class APIResource extends AbstractOpenShiftResource implements IOpenShiftConnection {
			...
			case STANDALONE:				
				this.standaloneCartridgeNames.add(
						new StandaloneCartridge(
								cartridgeDTO.getName(), cartridgeDTO.getDisplayName(), cartridgeDTO.getDescription(), cartridgeDTO.getTags()));
				break;
			case EMBEDDED:
	...
	@Override
	public List<String> getStandaloneCartridgesTags() throws OpenShiftException {
		...
		for(IStandaloneCartridge iCart : standaloneCartridgeNames){
			if(iCart.getTags() == null){
				continue;
			}
			for(String tag : iCart.getTags()){
				allTags.add(tag);
			}
		}
		
		return new ArrayList<String>(new HashSet<>(allTags));
	}

	@Override
	public List<IStandaloneCartridge> getSearchTagStandaloneCartridges(
			String tag) {
		// TODO Auto-generated method stub
		if (standaloneCartridgeNames.isEmpty()) {
			searchTagsCartridges(tag);
		}
		return standaloneCartridgeNames;
	}

	private void searchTagsCartridges(String tag) {
		// TODO Auto-generated method stub
		final Map<String, CartridgeResourceDTO> cartridgeDTOsByName = new GetCartridgesRequest().execute();
		for (CartridgeResourceDTO cartridgeDTO : cartridgeDTOsByName.values()) {
			// TODO replace by enum (standalone, embedded)
			switch (cartridgeDTO.getType()) {
			case STANDALONE:	
				if(cartridgeDTO.getTags() != null){
					if(cartridgeDTO.getTags().indexOf(tag) > -1){						
						this.standaloneCartridgeNames.add(
								new StandaloneCartridge(
										cartridgeDTO.getName(), cartridgeDTO.getDisplayName(), cartridgeDTO.getDescription(), cartridgeDTO.getTags()));
					}
